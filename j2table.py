...
def join():
    det = db.session.query(emplDet).join(emplFam, emplFam.emp_id == emplDet.emp_id).filter(emplDet.emp_id == 5).with_entities(emplDet.emp_id,emplFam.f_name,emplFam.m_name,emplFam.m_status,emplFam.ann_inc)
    res = []
    for row in det:
