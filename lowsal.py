...
def lows():
    #det=db.session.query(emp).filter(func.min(emp.salary)).all()
    det=db.session.query(func.min(emplDet.salary))
    det=db.session.query(emplDet).filter(emplDet.salary==det).all()
    res=[]
    for i in det:
        op = i.__dict__
